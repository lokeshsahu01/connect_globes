...
class AdminUsersForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ('account_id', 'username', 'first_name', 'last_name', 'email', 'mobile', 'user_role', 'user_department',
                  'sub_user', 'password', 'confirm_password')
...
class UserLoginForm(forms.Form):
    ...
    def clean(self):
        ...
        if not User.objects.get(username=cleaned_data['username']):
            raise forms.ValidationError("User is Not Exists !!!")
        else:
            if not User.objects.get(username=cleaned_data['username']).check_password(cleaned_data['password']):
